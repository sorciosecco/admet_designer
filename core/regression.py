...
def run_model_training():
    ...
    if settings.NPARA==False: ##### ALGORITHMS WITH DEFAULT PARAMETERS
        ...
        elif settings.MODEL=="AB": model=AdaBoostRegressor(base_estimator=None, n_estimators=50, learning_rate=1.0, loss='linear', random_state=None)
        ...
        #elif settings.MODEL=="rNN": model=RadiusNeighborsRegressor(radius=1.0, weights='uniform', algorithm='auto', leaf_size=30, p=2, metric='minkowski', metric_params=None, n_jobs=None)
        ...
        elif settings.MODEL=="GB": model=GradientBoostingRegressor(random_state=settings.SEED_GB, loss=parameters.loss_gb, learning_rate=parameters.learning_rate_gb, max_features=parameters.max_features, max_depth=parameters.max_depthGB, n_estimators=parameters.n_estimatorsGB, criterion=parameters.criterionGB)
        elif settings.MODEL=="AB": model=AdaBoostRegressor(random_state=settings.SEED, loss=parameters.loss, n_estimators=parameters.n_estimatorsAB)
        elif settings.MODEL=="ETR": model=ExtraTreesRegressor(random_state=settings.SEED, n_estimators=parameters.n_estimatorsETR, criterion=parameters.criterion, max_features=parameters.max_features, max_depth=parameters.max_depthETR)
